...
## 1. 프로젝트와 Source of Truth

`docs/`의 4개 문서가 기준이다. 코드와 충돌하면 문서가 우선한다.

| 문서 | 권한 범위 |
| --- | --- |
| Master GDD v0.2 | 핵심 루프, 주간 운영, 경제 해금 단계, 곡·발매·공연, 커리어 단계 |
| Character Master & System Architecture v1.1 | 캐릭터 15명(C01~C15) 정의, 성장 곡선, personality, contractProfile, Master/Save 구조, 시뮬레이션 순서 |
| Mobile IA & UX Specification v1.1 | 화면 구성, Dock, 주간 일정 슬롯 수, Back/Close 규칙, 몰입 플로우 |
| Visual Bible v1.0 | 아이소메트릭 월드와 캐릭터 비주얼 기준 |

- 기획/비주얼을 새로 창작하거나 재해석하지 않는다. 문서에 없는 판단은 코드에 `TODO(...)` 주석을 남기고 아래 "판단 필요 / TODO"에 기록한다.
- 문서가 **Lock**으로 표시한 항목(Character Master의 캐릭터 15명 정의 등)은 변경 대상이 아니다. 이름·포지션·visibleStats·hiddenStats·personality·contractProfileId를 수정하지 않는다 (`src/data/master/characters.ts`).
- **승인된 설계**와 **임시 수치**를 구분한다.
  - 승인된 설계: 문서에 있거나 사용자가 명시적으로 확정한 규칙.
  - 임시 수치: `src/data/master/prototypeBalance.ts`(`TODO(balance)`)와 `src/data/master/contractV1.ts`(`CONTRACT_V1_PROVISIONAL`). **영구 기획 규칙으로 기록하지 않는다.**
- 문서를 열 수 없는 환경이면 내용을 지어내지 말고, 확인 불가 항목으로 보고한다.

## 2. 아키텍처와 상태 불변식

- **Master → Engine → Save 단방향**. `src/data/master/`는 불변 정의, `SaveData`는 참조와 변화분만 보유한다. Save가 Master를 복제하지 않는다.
- 고정 캐릭터는 런타임에서도 `C01`~`C15`를 직접 참조한다. 이름 없는 세션만 `instanceId`를 쓴다.
- **역사 기록은 스냅샷이다.** `performanceHistory`, `releases[].result`, `careerHistory`는 당시 값으로 굳는다. Master 수치나 판정 계수를 바꿔도 과거 기록이 소급 변경되어서는 안 된다.
- 파생값은 `src/state/selectors.ts`에서 다시 계산하고 저장하지 않는다. **UI는 계산식을 쓰지 않는다.** 화면은 selector와 action만 호출하고, 같은 판정을 화면과 정산 코드에 중복 구현하지 않는다.
- SaveData 변경은 `src/state/actions/`에서만 한다.
- 주간 진행은 순수 함수 `simulateWeek(save)`가 한 번 계산하고 `commitWeek(outcome)`이 그 객체를 한 번만 적용한다. 화면 재진입·새로고침으로 보상이 두 번 지급되면 안 된다.
- 세이브 스키마 변경은 마지막 수단이다. 기존 구조로 고칠 수 없는 결함일 때만, **먼저 보고하고 승인받은 뒤** 바꾼다. 스키마를 늘릴 때는 가산적으로 하고 `ensureSaveDefaults`(`src/state/save/migrate.ts`)에서 구형 세이브를 채운다.
- 에셋이 없는 부분은 `[PLACEHOLDER_KEY]`로 둔다(`src/assets/registry.ts`). CSS로 아트를 흉내내지 않는다.
- 월드·카메라·타일 좌표·개발자 모드는 명시적 지시가 있을 때만 건드린다.

## 3. 경제와 계약 불변식

검증된 동작이다. 바꾸려면 근거가 필요하다.

- **급여는 체결된 계약의 `salary`에서 나온다.** 이미 합의한 계약을 현재 판정식으로 다시 값매김하지 않는다 (`weeklySalaryBurden`).
- 계약 주차는 **절대 주차**(`week + (year - 1) * 52`)로 저장한다. 연도가 바뀌어도 주수가 어긋나면 안 된다 (`absoluteWeek`, `src/state/sim/contract.ts`).
- 계약은 마지막 주까지 정상 지급한 뒤 종료한다. 만료 후에는 급여가 나가지 않고, 기존 계약과 예약된 재계약이 같은 주에 동시에 급여를 만들지 않는다. 주간 정산 순서는 `commitWeek`에서 **지출 적용 → 주차 증가 → `settleContracts`**이며, 이 순서가 위 불변식을 만족한다.
- 재계약은 `newContract.startWeek = oldContract.endWeek + 1`에서 시작한다. 기존 조건은 원래 만료 주까지 유지된다.
- 유효한 계약이 남아 있는 멤버를 `FORMER_MEMBER`로 바꾸거나 라인업에서 빼지 않는다. 만료한 멤버는 급여·출전 대상에서 빠지되 성장·참여 곡·과거 공연 기록은 남는다.
- **계약 판정은 결정론적이다.** `evaluateContract(characterId, salary, durationWeeks, rolePromise)`가 유일한 수락 판단 지점이고, 화면 표시·역제안·체결·재계약·역할 변경이 모두 이 함수를 통과한다. 난수나 시간 의존성을 넣지 않는다.
- 합의는 조건 스냅샷에 묶인다. 주급·기간·역할 중 하나라도 바뀌면 합의가 풀린다. 모든 역제안은 같은 판정으로 `accepted = true`가 확인된 조건이어야 한다.
- **예상치와 확정 거래를 구분한다.** 일정 화면의 "예상 지출"은 예측이고, 실제 차감은 정산 시점에 한 번 일어난다. 실행되지 않은 활동(치르지 않은 공연, 대상 없는 녹음)은 비용도 컨디션도 만들지 않는다.
- 기준 주급 15종, 시작 자금, 세션 비용, 공연 수익·제작비, 활동 비용, 시설 건설비는 지시 없이 바꾸지 않는다. 새로운 대출·미납·파산·위약금·랜덤 이탈 계산을 만들지 않는다.

## 4. 구현 절차

1. 고치기 전에 읽는다. 저장소 검색으로 실제 구현 위치를 찾고, 파일명이 예상과 다르다는 이유로 대체 파일을 만들지 않는다.
2. 지시받은 범위 안에서만 바꾼다. 관련 없는 리팩터링·아키텍처 개선·유틸리티 추가를 끼워 넣지 않는다.
3. 기존 함수와 프로젝트 관례를 먼저 쓴다. 잘 도는 시스템을 단순화·표준화를 이유로 교체하지 않는다.
4. 기획 충돌이나 새 게임 규칙이 필요하면 **임의로 정하지 말고 보고한다.**
5. 내부 rarity(`internalRarity`)는 UI가 읽지 않는다. 개발 문자열(에셋 키, TODO, 문서 절 번호)은 플레이어 화면에 노출하지 않는다.

## 5. 테스트와 인수

- 저장소 명령을 쓴다: `npm test`(vitest), `npm run typecheck`(tsc --noEmit), `npm run build`(typecheck + vite build).
- 브라우저 검증은 `tests/e2e/`의 스크립트를 쓴다. 실행 방법은 `tests/e2e/README.md`. 별도의 브라우저 테스트 프레임워크를 새로 만들지 않는다.
- 확인된 결함에는 재현 테스트를 먼저 만들고, 그 테스트가 기대한 이유로 실패하는 것을 확인한 뒤 고친다.
- 구현이 맞는데 커버리지만 없으면 테스트만 추가한다. 구현을 바꾸려고 테스트 기대값을 고치지 않는다.
- 세이브 적재/저장과 경계 조건(연도 넘김, 계약 마지막 주, 구형 세이브)을 함께 확인한다.
- 실행하지 않은 테스트를 통과했다고 보고하지 않는다. 실행한 명령과 실제 결과를 적는다.

## 6. Git과 변경 관리

- 편집 전에 `git status --short --branch`로 작업 트리를 확인한다.
- `git reset` / `git clean` / `git stash` / `git restore` 등으로 기존 사용자 변경을 지우지 않는다.
- 맡은 작업에 속한 파일만 stage 한다. 관련 없는 파일이나 추적되지 않는 산출물을 커밋에 넣지 않는다.
- **commit/push/배포는 사용자가 요청할 때만.** 커밋 해시를 만들기 위한 커밋을 하지 않는다.

## 7. 중단 조건과 보고

다음이면 해당 항목의 작업을 멈추고 보고한다. 우회 구현이나 임의의 기획 결정을 하지 않는다.

- 문서에 확정된 규칙과 충돌하는 변경이 필요할 때
- 기존 세이브나 과거 계약·공연 기록을 파괴적으로 고쳐야 할 때
- 계약 계수·기준 주급·경제 밸런스를 바꿔야 풀리는 문제일 때
- 주전 약속 검증에 새로운 라인업 규칙·계약 자격 정책·페널티 시스템이 필요할 때
- 선언한 범위를 벗어나는 아키텍처 변경이 필요할 때
- 기존 사용자 변경과 섞이거나 덮어쓸 위험이 있을 때
- 필요한 문서나 구현 파일이 없어 검증이 불가능할 때

보고할 때는 막힌 작업 항목, 충돌하는 요구사항, 관련 파일을 적는다. **구현한 것과 제안에 그친 것을 분리해서** 적는다. 한 항목이 막혀도 독립적인 나머지 작업은 계속한다.
...
### 2026-09-19 — 계약 협상 PHASE 1 (기존 협상 기능 개선)
- **진단**: 수락 판정은 주급 하나(`salary / baseSalary >= 0.75`)만 봤고 계약 기간·역할은 전혀 반영되지 않았다. 그런데 화면은 "거의 확실 / 가능성 높음 / 반반 / 어려움" 4단계 **확률 어휘**를 썼다 — 확률은 굴리지 않으므로 "반반"도 100% 수락이었다. 역제안은 문구뿐이고 구체적 조건이 없었다. **수락 후 조건을 바꿔도 합의가 유지되어, 합의한 것과 다른 조건이 저장되는 버그**가 있었다.
- 판정을 `state/sim/contract.ts` 한 곳으로 모았다. 화면 표시·역제안·실제 체결이 모두 `acceptsTerms()`를 통과한다.
- 확률 어휘를 제거하고 단정적으로 표시한다: "지금 조건이면 받아들인다 / 받아들이지 않는다" + "주급 ₩X 이상이면 받아들인다". 기간·역할이 수락 여부를 바꾸지 않는다는 점도 화면에 밝혔다.
- 역제안은 실제로 수락되는 조건만 최대 2개(받아들이는 최저 주급 / 기준 주급) 제시하고, 플레이어가 고른 기간·역할은 그대로 유지한다. 최저 주급은 주급 조절 단위에 맞춰 올림해 그대로 고를 수 있다.
- 합의는 **조건 스냅샷**에 묶인다. 주급·기간·역할 중 하나라도 바뀌면 합의가 풀리고 "조건이 바뀌었다"가 뜨며 체결 버튼이 사라진다. 저장되는 계약은 합의한 스냅샷 그 자체다.
- 계약 화면에 현재 주간 급여 / 영입 후 주간 급여 / 보유 자금 / 급여만 지급할 때 버틸 주수를 표시하고, 활동비·수익을 뺀 단순 추정치임을 밝혔다.
- **수치 변경 없음**: 기존 임계값 0.75를 `prototypeBalance.contract.acceptSalaryRatio`로 옮겼을 뿐 값은 같다. 기준 주급·시작 자금·공연 수익은 그대로이고, 기간·역할에 급여 할인 효과를 넣지 않았다.
- 검수: 단위 테스트 118개 + 브라우저 7종(`contract` 신규 포함) 통과.

### 2026-09-19 — CONTRACT SYSTEM V1
- **판정**: `salary >= base * 0.75` 단일 판정을 없애고 `evaluateContract(characterId, salary, durationWeeks, rolePromise)` 순수 함수로 교체했다. 반환값은 `{ accepted, reason, counterOffers }` + 요구 주급. 결정론적이며 확률을 굴리지 않는다.
  - 기간은 **riskTolerance**, 역할은 **ego**로 갈린다(Character Master §09 personality). 같은 장기/주전 조건이 누구에게는 웃돈이고 누구에게는 할인이라 **일괄 할인이 아니다**. 선호 기간은 contractProfile의 `preferredDurationWeeks`를 쓴다.
  - 기준 주급 15종은 그대로 두고 배수만 만든다. 가중치는 전부 `CONTRACT_V1_PROVISIONAL` 시험값.
  - 역제안 최대 2개, 전부 같은 함수로 `accepted=true`가 확인된 조건. 주급을 올리는 길과 기간·역할을 바꾸는 길을 함께 준다.
- **계약 기간**: 모든 계약 주차를 **절대 주차**(연도 반영)로 저장한다. 26/52/104주가 정확히 그 주수만큼 가고 마지막 주 정산까지 정상 지급한 뒤 종료한다. 만료 시 멤버·라인업·급여·공연 대상에서 빠지고 `FORMER_MEMBER`가 되어 오디션 후보로 되돌아가지 않는다. 성장·참여 곡·과거 공연 기록은 그대로 남는다.
- **재계약**: 마지막 4주(`renewalWindowWeeks`)에만 열린다. 기존 계약은 끝까지 기존 조건이고, 새 계약은 `endWeek + 1`에 시작해 기간이 중복되지 않는다.
- **주전 기용 약속**: 공연 기록에 **실제 출전 멤버 ID**를 남기고, 계약 시작 이후 완료된 공연 최근 4회 중 결장 1회까지 허용한다. 출전 기록이 없는 과거 공연은 가짜로 채우지 않고 건너뛴다. 공연 준비 화면이 현재 편성으로 위반을 미리 계산해, **고칠 편성이 있으면 공연 확정을 막고** 없으면 진행을 허용하되 `contractExceptions`에 기록한다(페널티 없음).
- **역할 변경**: 같은 판정을 다시 통과해야 하고 다음 주부터 적용된다. 만료일은 유지된다. 주전 최저선 주급으로 서포트로 갈아타는 무임승차는 판정에서 막힌다.
- 화면은 늘리지 않았다. 계약 협상 화면 하나가 첫 계약 / 재계약 / 역할 재협상을 모두 맡고, 진입은 기존 계약 화면의 버튼이다.
- 검수: 단위 테스트 137개 + 브라우저 7종 통과.
...
- ~~계약 기간·역할이 판정에 반영되지 않음~~ 해결(CONTRACT V1).
- **수락 임계값 0.75**: 기존 화면이 쓰던 값을 그대로 옮긴 것이며 문서 근거는 없다(`TODO(balance)`).
- **CONTRACT_V1_PROVISIONAL 시험값**: `durationStepWeight 0.06`, `supportRoleWeight 0.30`, `baseAcceptRatio 0.75`, 요구 비율 상·하한 `0.5~1.6`, `renewalWindowWeeks 4`, 주전 판정 `최근 4회 / 결장 1회`. 전부 이번에 도입한 V1 시험 규칙이며 기획 확정값이 아니다.
- **주전 약속 위반의 결과 (미해결)**: 지킬 수 있는 편성이 없으면 공연을 막지 않고 `contractExceptions`에 기록만 한다. 불만·이탈·위약금 같은 결과는 새 경제/페널티 시스템이 필요해 임의로 만들지 않았다.
- **동일 포지션 주전 중복 계약**: 실제 편성으로 약속 이행이 불가능한 경우에만 공연 준비 화면에서 걸린다. 포지션당 주전 1명 같은 인위적 제한은 두지 않았다.
- **라인업 변경에 따른 라이브 숙련도 처리**: 후속 규칙으로 남아 있다.
